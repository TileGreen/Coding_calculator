...
#         # Check if throughput is physically achievable
#         if throughput_kg_h > Q_max * 1.1:  # Allow 10% over capacity
#             logger.warning(f"Candidate {cand_map.get('name', f'#{idx}')} cannot achieve {throughput_kg_h} kg/h (max: {Q_max:.1f} kg/h)")
#             continue  # Skip this candidate
        
#         # Actual fill factor based on desired throughput
#         fill_eff = float(throughput_kg_h) / float(Q_max)
        
#         # Ensure fill factors are physically realistic
#         fill_eff = max(0.3, min(1.2, fill_eff))  # Limit to 30-120%
        
#         D_m = inp.D_mm / 1_000.0                     # [m]
#         # Use candidate's RPM if available, otherwise use global RPM
#         candidate_rpm = inp.screw_speed_rpm if inp.screw_speed_rpm is not None else rpm
#         V_surf = np.pi * D_m * candidate_rpm / 60.0   # [m/s]
#         w_slot = np.pi * D_m                         # [m]
...
#         # 3) Effective depths with realistic constraints
#         MIN_DEPTH = 0.002  # Increased to 2mm (from 0.1mm) for physical realism
        
#         # Feed zone uses actual fill
#         h_f_eff_m = max((h_f_mm * fill_eff) / 1_000.0, MIN_DEPTH)
        
#         # Compression zone uses fill heuristic with realistic limits
#         comp_fill = get_comp_fill(fill_eff, h_c_mm, inp.D_mm)
#         comp_fill = max(0.4, min(1.0, comp_fill))  # Limit to 40-100%
...
#         V_feed = Q_m3_s / max(A_feed, 1e-10)
#         V_comp = Q_m3_s / max(A_comp, 1e-10)
#         V_metr = Q_m3_s / max(A_metr, 1e-10)
        
#         # Check for unrealistic velocities (> 0.1 m/s is typical minimum)
#         if V_feed < 0.01 or V_comp < 0.01 or V_metr < 0.01:
#             logger.warning(f"Candidate {cand_map.get('name', f'#{idx}')} has very low axial velocity (< 1 cm/s)")
        
#         geom["t_feed_s"] = (inp.L_feed / 1_000.0) / max(V_feed, 0.01)
#         geom["t_comp_s"] = (inp.L_compression / 1_000.0) / max(V_comp, 0.01)
#         geom["t_metr_s"] = (inp.L_meter / 1_000.0) / max(V_metr, 0.01)
...
#             logger.error(f"Flow solver failed for candidate {cand_map.get('name', f'#{idx}')}: {e}")
#             continue  # Skip this candidate

#         # 6) Check for unrealistic pressure gradients
#         max_allowed_dpdx = 15e6  # 15 MPa/m
#         if (abs(dpdx_feed) > max_allowed_dpdx or 
#             abs(dpdx_comp) > max_allowed_dpdx or 
#             abs(dpdx_metr) > max_allowed_dpdx):
#             logger.warning(f"Candidate {cand_map.get('name', f'#{idx}')} has unrealistic pressure gradients")
        
#         # 7) Actual mass-based fill factors
#         mass_C_feed = Q_C_feed * rho_melt * 3_600.0
#         mass_C_comp = Q_C_comp * rho_melt * 3_600.0
#         real_feed_fill = throughput_kg_h / max(mass_C_feed, 1e-10)
#         real_comp_fill = throughput_kg_h / max(mass_C_comp, 1e-10)

#         # 8) Mixing metrics
#         tau_max = max(tau_feed, tau_comp, tau_metr)
#         gamma_int = gamma_feed + gamma_comp + gamma_metr

#         # Check for minimum shear stress for dispersion
#         if tau_max < 3.5e5:  # 0.35 MPa
#             logger.info(f"Candidate {cand_map.get('name', f'#{idx}')} has insufficient shear stress for dispersion: {tau_max/1e6:.2f} MPa")
