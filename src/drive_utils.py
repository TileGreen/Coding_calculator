...
_THROUGHPUT_POWER_COEFF_DEFAULT = MotorSizingInputs().throughput_power_coeff
...
def _torque_from_throughput(
    throughput_kg_h: float,
    rpm_screw: float,
    throughput_power_coeff: float = _THROUGHPUT_POWER_COEFF_DEFAULT,
) -> float:
    """Return screw torque [N·m] via empirical :math:`P = k_{power} \dot m`."""
    P_kW = throughput_power_coeff * throughput_kg_h
    P_W = P_kW * 1_000.0
...
def motor_from_screw(*args, **kwargs) -> Dict[str, float]:
    ...
    2. **Keyword throughput**: throughput_kg_h=…, rpm=…
       [, gear_ratio, gear_eff, throughput_power_coeff]

    Notes
    -----
    The throughput-based route uses an empirical throughput-to-power coefficient
    in units of [kW / (kg·h⁻¹)]. The legacy keyword ``kv`` is still accepted as
    an alias for backward compatibility, but it is **not** a motor voltage
    constant.
    """
    ...
    if args:
        if len(args) < 3:
            raise TypeError(
                "motor_from_screw positional form needs ≥3 args: torque_screw, rpm_screw, gear_ratio"
            )
        ...
        throughput_power_coeff = kwargs.pop(
            "throughput_power_coeff",
            kwargs.pop("kv", _THROUGHPUT_POWER_COEFF_DEFAULT),
        )
        ...
        if torque_screw is None:
            ...
            torque_screw = _torque_from_throughput(
                throughput,
                rpm_screw,
                throughput_power_coeff,
            )
...
def select_motor(
    torque: Union[float, Dict[str, float]],
    *,
    rpm: float | None = None,
    service_factor: float = _SERVICE_FACTOR_DEFAULT,
